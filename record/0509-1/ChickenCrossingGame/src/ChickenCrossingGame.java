...
public class ChickenCrossingGame extends JPanel implements ActionListener, KeyListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (!gameOver) {
            ...
            if (chickenY < 0) {
                ...
                chickenY = roads.get(roadCount - 1).y + objectHeight; // 重新設定在底部綠色安全道路下方
                carSpeed = 5 + level;
                timeElapsed = 0;
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (!gameOver) {
           ...
           // if (chickenY < 0) chickenY = 0; // 防止小雞跑到畫面頂部以上
            if (chickenY > roads.get(roadCount - 1).y + objectHeight) chickenY = roads.get(roadCount - 1).y + objectHeight; // 防止小雞跑到綠色道路下方過多
        } else {
